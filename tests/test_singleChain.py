...
def test_sc_sequence():
    ...
    assert name_list == ['NAA','NAT','NAG','NAC'] and chain.print_seq() == 'ATGC'
...
def test_bonds_twoResidue():
    ...
    assert len(bonds) == 3 and set([firstBond,secondBond,thirdBond]) == set([frozenset([pl[0],pl[1]]),frozenset([pl[2],pl[3]]),frozenset([pl[0],pl[2]])])
