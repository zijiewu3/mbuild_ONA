    ...
    class ONA_box(mb.Compound):
        def __init__(self,sequences = [],dim = [0,0,0]):
            ...
            if len(sequences) >0 and dim[0]*dim[1]*dim[2] != len(sequences):
                ...
                print('Number of chains and dimension do not match. Reconfiguring dim to '+str(dim))
            seq_num = 0
        ...
        def visualize(self):
            ...
            view = py3Dmol.view()
            rad = {'_BBA':0.05,'_BBC':0.05,'_BBG':0.05,'_BBT':0.05,'_HBA':0.022,'_HBC':0.022,'_HBG':0.022,'_HBT':0.022}
